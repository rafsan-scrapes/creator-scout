...
- 2026-09-10 — Phase 8 step 5 done: new `server/channel-resolver.ts` with `parseChannelIdentifier` (pure parse, exported for tests) + `resolveChannelIdentifier` returning `{ status: "resolved", channel: { channelId, channelUrl, channelName }, quotaCost } | { status: "already-in-history" }`. Cases: (a) bare `UC`+22 ID or `/channel/` URL → `isChannelKnown` first, `"already-in-history"` with zero API calls if known, else `channels.list?id=` (1 unit) to confirm + fetch display name; (b) `@handle` bare/URL → `channels.list?forHandle=` with canonical `@` form (per API docs, `@` optional — 1 unit); (c) `/user/` URL → `forUsername` (1 unit); (d) `/c/` URL → `search.list?type=channel` top match (100 units, `quotaCost` returned so callers can surface it — step 8 UI must note this cost under the input). (e) All calls go through `fetchYouTubeJsonWithQuota` from `server/youtube.ts` (now exported, along with `YOUTUBE_BASE_URL`; no separate quota path). Failures: `ChannelResolveError("invalid-input" | "channel-not-found")` for bad/missing channels; quota/key/network ProviderErrors propagate untouched for the step 6 route to map. New `server/channel-resolver.test.ts` (18 tests, mocked fetch): parse branches incl. watch/shorts/non-youtube/over-long rejects, per-case API params, already-in-history skips fetch, not-found/invalid errors, usage accounting (1 and 100 units), no-key and mid-resolution quota errors. Full suite `npm test` 46/46 pass (28 existing + 18 new), `npm run check` clean.
- 2026-09-10 — Phase 8 step 6 done: `POST /api/history/manual-add` in `server/routes.ts` (body `{ input }` validated by new `manualAddRequestSchema` in `shared/schema.ts` — trim, 1..500 chars matching the resolver cap, strict). Flow: resolve via step 5 → resolver-level `"already-in-history"` → 409; else re-check `isChannelKnown` on the resolved ID (covers handle/username/custom paths) → 409; else `addManualChannel` (its own no-op is belt-and-braces) → 201 with `{ status: "added", channel }` (fresh `getChannel` row). Own rate-limiter instance, same budget as scout (10/min — real API calls). Every outcome carries a `status` discriminator: `added` / `already-in-history` (409) / `invalid-input` (400, zod + resolver) / `channel-not-found` (404) / `quota-exhausted` (429, from provider payload category) / `provider-error` (other provider failures, full payload preserved); `Cache-Control: no-store`. New `server/manual-add.test.ts` (7 tests, ephemeral-port Express app, YouTube mocked at global fetch with localhost passthrough, real DB dedup with throwaway fake IDs — no `isChannelKnown` stub by design): valid add incl. record shape, dup bare ID with zero fetch calls, dup-via-handle caught post-resolution, invalid format with zero fetch, empty/blank rejects, not-found 404, no-key quota 429. Full suite `npm test` 53/53 pass, `npm run check` clean, real DB back to 24 rows (test fakes deleted).
- 2026-09-10 — Phase 8 step 7 done: `GET /api/history` in `server/routes.ts` returns `{ channels: listHistory() }` (newest-first) with `Cache-Control: no-store` — live, mutable state, same reasoning as Phase 6.1 W5. No rate limiter by design (local DB read, zero YouTube API cost). Covered in `server/manual-add.test.ts`: seeded manual row comes back first with `source: "manual"` / NULL keyword, full list monotonically newest-first, `no-store` header. Full suite `npm test` 54/54 pass, `npm run check` clean.
- 2026-09-10 — Phase 8 step 8 done: new `client/src/pages/history.tsx` (page only — sidebar/route wiring is step 9). Manual-add card (input + Add button, disabled while in flight or blank, spinner + "real network call" live status) above the history table (Channel name linked to URL, "Found via search" / "Manually added" badge, Matched Keyword badge or —, Added date; newest-first; loading / load-error / empty states). Inline feedback per outcome via the route's `status` discriminator: green "Added to history", amber "Already in history", destructive for invalid-input / channel-not-found / quota-exhausted / provider-error (with server suggestion appended when present). Static one-line `/c/` cost note under the input per step 5d (~100 units vs 1). Follows scout.tsx conventions (shadcn Card/Input/Button/Badge/Alert/Table, lucide icons, direct fetch with credentials, data-testids). `npm run check` clean, `npm run build` succeeds (client + server bundles), `npm test` 54/54 pass.
- 2026-09-10 — Phase 8 step 9 done: navigation wired — "History" entry (lucide `History` icon, `/history`, `data-testid="link-history"` via the existing generator) added to the Tools menu in `client/src/components/app-sidebar.tsx` alongside Scout (Settings stays in the footer, unchanged), plus `<Route path="/history" component={HistoryPage} />` in `client/src/App.tsx`. `npm run check` clean, `npm test` 54/54 pass, `npm run build` succeeds with a fresh client bundle.
- 2026-09-10 — Phase 8 step 10 done (tests were built incrementally with each step; this pass closed the gaps and hardened the suite). Coverage audit: resolver branches → `server/channel-resolver.test.ts` (18 tests: parse all 5 kinds + 13 invalid shapes, per-case API params, already-in-history with zero fetch, not-found/invalid errors, 1- and 100-unit usage accounting, no-key + mid-resolution quota); `addManualChannel` duplicates → NEW `server/db.test.ts` (3 tests on the real DB with throwaway fake IDs: fresh `"added"` shape, repeat `"already exists"` preserving `added_at`/name, pre-existing search row untouched); `GET /api/history` → newest-first + `no-store` test; `POST /api/history/manual-add` → 9 tests incl. NEW route-level 403-quotaExceeded-mid-resolution → 429 `quota-exhausted`. Hardening: the full suite flaked once (58-run: resolver usage assertion failed standalone-clean) — root cause is parallel test processes sharing the single real `data/scout.db` (usage counters + silent-catch resets assume sequential files), so `package.json` test script is now `tsx --test --test-concurrency=1 ...`. After the fix: two consecutive full runs 58/58 pass, `npm run check` clean, real DB back to 24 rows (all fakes deleted).